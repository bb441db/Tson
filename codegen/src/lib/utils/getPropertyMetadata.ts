...
import {TSON_PROP} from "../constants";
...
export default function getPropertyMetadata(program: Program, member: ParameterDeclaration | PropertyDeclaration): TsonPropData {
        ...
        if (member.decorators && member.decorators.length > 0) {
            for (const decorator of member.decorators) {
                if (isCallOrNewExpression(decorator.expression)
                    ...
                    && decorator.expression.expression.text === TSON_PROP) {
                    return decorator;
                }
            }
        }
